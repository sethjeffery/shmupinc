...
export const PLAYFIELD_BASE_WIDTH = 360;
export const PLAYFIELD_BASE_HEIGHT = 640;
export const PLAYFIELD_CORNER_RADIUS = 18;
export const PLAYFIELD_MARGIN = 10;

export const computePlayArea = (width: number, height: number): Phaser.Geom.Rectangle => {
  const availableWidth = Math.max(0, width - PLAYFIELD_MARGIN * 2);
  const availableHeight = Math.max(0, height - PLAYFIELD_MARGIN * 2);
  const scale = Math.min(availableWidth / PLAYFIELD_BASE_WIDTH, availableHeight / PLAYFIELD_BASE_HEIGHT);
